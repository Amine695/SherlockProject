		...
		if (synchro==1)
		{
			switch (gbuffer[0])
			{
				...
				case 'V':
					...
					if(tableCartes[i][j] == 0 || tableCartes[i][j] == 100 || tableCartes[i][j] == -1)
					{
						tableCartes[i][j] = k;
					}
					break;

				// Message 'W' : le joueur recoit l'id du joueur gagnant et la carte coupable
				case 'W':
					sscanf(gbuffer, "W %d %d", &i, &j);
					printf("%s a gagné ! La carte coupable était : %s.\n",gNames[i],nbnoms[j]);
					quit = 1;
					break;

				// Message 'E' : le joueur recoit l'id du joueur éliminé
				case 'E':
					sscanf(gbuffer, "E %d %d", &i, &j);
					if(i == gId)
					{
						printf("Vous avez été éliminé ! %s n'est pas la carte coupable.\n",nbnoms[j]);
					}
					else
					{
						printf("%s a été éliminé ! %s n'est pas la carte coupable\n",gNames[i],nbnoms[j]);
					}
					break;
			}
