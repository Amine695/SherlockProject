        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				case 'C':
					...
					if(nbClients==4)
					{	
						//on distribue les cartes & symboles aux 4 joueurs
						for(int k = 0; k < 4; k++)
						{
							...
							//Message 'V' : le joueur recoit une valeur de tableCartes (Symboles)
							for (int p = 0; p < 8; p++)
							{
								sprintf(reply, "V %d %d %d", k, p, tableCartes[k][p]); 
								sendMessageToClient(tcpClients[k].ipAddress, tcpClients[k].port, reply);
							}
						}

						// On envoie enfin un message a tout le monde pour definir qui est le joueur courant=0
						sprintf(reply,"M %d",joueurCourant); //Message 'M' : le joueur recoit le n° du joueur courant
						broadcastMessage(reply);
		...
		else if (fsmServer==1)
		{
			switch (buffer[0])
			{
				case 'G': //Guilt 
					...
					if(i != deck[12]) //le joueur n'a pas trouvé la carte coupable
					{	
						sprintf(reply, "E %d %d", id, i);
						broadcastMessage(reply);
						TrackEliminatedPlayer[id] = 1;
						count = 0;

						for(int k = 0; k < 4; k++)
						{
							if(TrackEliminatedPlayer[k] == 0)
							{
								count++;
							}
						}

						if(count == 1) //Il ne reste plus qu'un joueur, celui-ci est le gagnant 
						{	
							for(int k = 0; k < 4; k++)
							{
								if(TrackEliminatedPlayer[k] == 0)
							...
							sprintf(reply, "W %d %d", joueurCourant, deck[12]);
							quit = 1;
						}

						else //la partie continue avec un joueur en moins
						{	
							count = joueurCourant + 1;
							...
							while(1)
							{
								...
								if(TrackEliminatedPlayer[count] == 0)
								{
									joueurCourant = count;
									break;
								}
					...
					else{ //le joueur a trouvé la carte coupable
				...
				case 'O': //Others
                    ...
                    sscanf(buffer, "O %d %d", &id, &i);

					for (int k = 0 ; k < 4 ; k++)
					{
						if(k != id)
						{
							if(tableCartes[k][i] > 0)
							{
								sprintf(reply, "V %d %d 100", k, i); //le joueur k a le symbole i 
							}

							else 
							{
								sprintf(reply, "V %d %d 0", k, i); //le joueur k n'a pas le symbole i
							}
